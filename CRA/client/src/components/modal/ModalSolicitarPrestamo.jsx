...
const ModalSolicitarPrestamo = ({ active, setActive, titulo }) => {
  ...
  const [garante, setGarante] = useState({ identification: "", name: "" });
  ...
  const agregar = async () => {
    ...
    if (garante.name.length < 1)
      return alert(
        "Error",
        "Por favor escribir el nombre de la persona u objeto de garantia",
        "error"
      );
          ...
          <div className=" p-3 rounded ">
            <div className="contenido">
              ...
              <div className="border border-gray-500 p-3 grid gap-2">
                <h2 className="m-auto text-xl">Garante: </h2>
                <input
                  type="text"
                  className="border border-gray-700 rounded-md text-center m-auto"
                  placeholder="Escribe la cedula del garante"
                  onChange={(e) => setGarante({ ...garante, identification: e.target.value })}
                />
                <label className="flex items-center mt-3 text-red-600 text-sm">
                  {" "}
                  * obligatorio
                </label>
                <input
                  type="text"
                  className="border border-gray-700 rounded-md text-center m-auto"
                  placeholder="Escribe el nombre o el articulo"
                  onChange={(e) => setGarante({ ...garante, name: e.target.value })}
                />
              </div>
            </div>
            <div className=" mt-12">
