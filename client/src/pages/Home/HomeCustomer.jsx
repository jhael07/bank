...
const HomeCustomer = () => {
  ...
  const { nombre, apellido, cuentabancos } = JSON.parse(
    decrypt(account, import.meta.env.VITE_SECRET_KEY)
  );
  console.log(JSON.parse(decrypt(account, import.meta.env.VITE_SECRET_KEY)));
      ...
      {session.session ? (
        <div className="bg-white container-page">
          ...
          <div className="bg-white p-4 w-11/12 m-auto">
            <div
              className={"w-full flex m-auto p-2 mb-6 mt-20 justify-center text-2xl text-gray-700"}
            >
